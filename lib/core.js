...
async function getAYearData(params) {
   params.pc = await findPC(params.stnNum, params.nccObsCode)
   if (params.pc == null) {
      return Promise.reject('p_c not found')
   }
   log('Found p_c: ' + params.pc)
...
async function pasrseZipResponse(res, params) {
   ...
   return new Promise((resolve, reject) => {
      csvParse(csvContent, opts, function (err, csvDatas) {
         const result = csvDatas.map(v => fmtFunc(v, params))

         fs.rmdirSync(tmpDir, { recursive: true })

         log('Got entries: ' + result.length)
...
function format2(csvdata, params) {
   ...
   return {
      ...
      year: csvdata.Year,
      data: {
         annual: csvdata.Annual,
         months: monthNames.map(m => csvdata[m])
      }
   }
}

function findPC(stationNum, nccObsCode) {
   const params = {
   ...
   return new Promise((resolve, reject) => {
      axios.get(reqParams.base + reqParams.pathPc, { params: params })
         .then(function (res) {
            let pc = parsePCHtml(res.data)
            return pc ? resolve(pc) : reject('p_c not found')
         })
...
function parsePCHtml(data) {
   let pc = null
   let isFoundRow = false
   const parser = new htmlParser.Parser(
      {
         onopentagname(name) {
            if (name === 'tr') isFoundRow = true
         },
         ontext(text) {
            if (isFoundRow) pc = text
         },
         onclosetag(name) {
            if (name === 'tr') isFoundRow = false
         }
      }
   )
